...
def design_metrics(trees: list[dict], site_area_m2: float | None, cost_eur: float | None) -> dict:
    """Urban-design metric bundle for one site's planting.

    Args:
        trees: placed trees (each with 'species').
        site_area_m2: plantable/site area for canopy-cover %, or None.
    ...
    if site_area_m2 and site_area_m2 > 0:
        cover_pct = round(100.0 * canopy / site_area_m2, 1)
        out["canopy_cover_pct"] = cover_pct
        out["canopy_target_pct"] = CANOPY_TARGET_PCT
        out["canopy_gap_to_target_pct"] = round(max(0.0, CANOPY_TARGET_PCT - cover_pct), 1)
        # Geometric estimate of ambient relief from the canopy-cover gain.
        out["est_air_temp_drop_c"] = round(cover_pct * _DEG_C_PER_CANOPY_PCT, 2)
    return out
