...
We evaluated a ~0.5 ha plaza on the **live** Infrared.city backend. The greedy engine placed **20 trees** drawn from the plantable palette. The measured cooled-footprint profile reported **2,573 m² cooled by at least 0.5 °C**, a **mean UTCI reduction of 1.63 °C** over cooled ground, and a cost efficiency of **€77.7 per m² cooled**. This is the system's measured proof point: real geometry, real species screening, and a real UTCI A/B simulation on the same site.

The showcase was regenerated on the current palette: its 20-tree layout contains **zero** *Platanus* × *acerifolia* and zero invasive species (8 *Tipuana tipu*, 7 *Celtis australis*, 5 *Styphnolobium japonicum*), at 8 m spacing with per-tree maturity, confirming the plantability gate is effective end-to-end. Every output of the current pipeline — the showcase, the evaluate endpoint, and the €1,000,000 plan — is London-plane- and invasive-free.
...
2. **Coverage proxy versus measured cooling** *(partially remediated; see `coolspend/shade_proxy.py`).* The greedy objective still weights demand by the *baseline* UTCI field, so the per-step marginal gain is a coverage proxy and the final layout is validated by one true UTCI run. We have, however, added a sim-free first-order *physical* alternative — a ray-cast sun-blockage (shade-gain) proxy that responds to actual solar geometry and existing obstacles, is submodular (preserving the greedy guarantee), and supplies the portfolio's sim-free cooling estimate. Wiring it as the per-step greedy weight (so placement *order* itself is shade-gain-driven) is the remaining step; the abstraction and physics are in place.
