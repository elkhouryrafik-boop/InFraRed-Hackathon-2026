...
where each demand cell is covered not by mere crown overlap but by **shade-gain**: the fraction of the sampled July sun positions whose crown shadow actually falls on the cell (`coolspend/shade_proxy.py`). A tree therefore earns a cell only if its shadow reaches it — placing trees *south* of midday hotspots, not merely on top of them — and a cell already shaded contributes only the *new* blockage (per-cell captured gain is capped at 1.0, so double-shading yields nothing). The algorithm is a cost-benefit greedy: at each step it computes, for every (slot, species) pair, the marginal `Σ weight(cell) × (shade_gain − already-captured)`, divides by the tree's cost to obtain gain-per-euro, and selects the maximum, breaking ties first by the species' cooling score and then by absolute marginal gain (for determinism). Selection stops when no affordable slot yields positive marginal gain or the budget is exhausted. A `use_shade_gain=False` flag falls back to the legacy binary crown-coverage proxy.

This inherits the (1 − 1/e) submodular guarantee (Nemhauser et al., 1978) in its cost-benefit form (Leskovec et al., 2007) — the shade-gain sets union exactly like coverage sets, so submodularity is preserved. A live A/B confirmed the shade-gain ordering does not regress the headline: on the plaza it cooled a measured **2,779 m²** at **€72.0/m²** versus the binary-coverage layout's 2,573 m² at €77.7/m² (a slightly lower mean ΔUTCI, 1.48 vs 1.63 °C, because the larger footprint includes more marginal-benefit edge cells). Two refinements temper a pure shade-maximiser: an **anti-monoculture constraint** caps any single species at 40% of the placement after a four-tree grace period, and the per-tree crown used for coverage is capped to the site (Section 4.4).
...
The "years after planting" experience is, however, *species-specific*, and the system models it with a proper growth curve rather than the flat establishment ramp. Each species' canopy follows a **Chapman–Richards** growth function (`coolspend/growth.py`, mirrored client-side in `web/src/lib/growth.ts`) — the standard sigmoid form used in forestry — anchored to two real per-species values: the mature crown diameter (from the inventory) and the time-to-near-mature-canopy assigned from the species' growth-rate band (fast = 20, medium = 30, slow = 40 years, consistent with i-Tree establishment ranges and the climate-responsive "continuous canopy at maturity, 20–30 years"). The single rate parameter is solved so the crown reaches 95% of its mature diameter at the species' maturity year, starting from a realistic large-caliper nursery crown; shade scales with crown *area*, so the cooling fraction is the squared crown ratio. The model invents no per-species coefficients — only the sigmoid form and the two anchors — and is structured to accept measured i-Tree/Pretzsch coefficients later without changing call sites. The age slider now grows each tree's canopy disk on its *own* curve: a fast *Tipuana tipu* fills its crown roughly twice as quickly as a slow *Cercis siliquastrum*, both visibly and in the per-tree inspector. The economic establishment ramp in the cost model (a separate linear discount for net-present-value accounting) is retained but is no longer the basis for the visual or canopy-area growth.
...
Both paths converge on the same validation step. The chosen layout(s) are submitted to the Infrared.city engine for a true UTCI evaluation: one baseline simulation (uncounted) plus up to three intervention simulations, hard-capped by a simulation budget. The thermal-relief surrogate used inside NSGA-II is an analytical proxy, explicitly *not* a measured value. Its uncertainty is no longer assumed: a live calibration study (`coolspend.calibration`, 11 simulations, summary committed to `coolspend/data/calibration_summary.json`) measured the surrogate against real UTCI deltas, yielding an empirical 95% band of **±0.78 °C** (1.96 × RMSE 0.398 °C) — roughly five times tighter than the previously assumed ±4 °C. Critically, the same study found the surrogate to be a *weak magnitude predictor* (R² = −0.946, biased low) but a *moderate ranker* (Spearman ρ = 0.69, two of three top-three configurations retained under real UTCI). We therefore treat the surrogate strictly as a ranking heuristic and take all reported cooling magnitudes from the live sim — which is precisely how the pipeline is wired.
...
We evaluated a ~0.5 ha plaza on the **live** Infrared.city backend. The greedy engine (shade-gain weighting) placed **20 trees** drawn from the plantable palette. The measured cooled-footprint profile reported **2,779 m² cooled by at least 0.5 °C**, a **mean UTCI reduction of 1.48 °C** over cooled ground, and a cost efficiency of **€72.0 per m² cooled**. This is the system's measured proof point: real geometry, real species screening, and a real UTCI A/B simulation on the same site.

The showcase was regenerated on the current palette and shade-gain placement: its 20-tree layout contains **zero** *Platanus* × *acerifolia* and zero invasive species (8 *Tipuana tipu*, 5 *Styphnolobium japonicum*, 4 *Cercis siliquastrum*, 3 *Melia azedarach*), at 8 m spacing with per-tree maturity, confirming the plantability gate is effective end-to-end. Every output of the current pipeline — the showcase, the evaluate endpoint, and the €1,000,000 plan — is London-plane- and invasive-free.

### 6.2 City-wide €1,000,000 portfolio

The multi-site allocator committed **€990,000** across **7 geographically separated sites**, funding **99 trees** in total (the geographic-spread constraint exhausted the top-10 candidate pool before the final ~€10,000 could reach a sufficiently distant eighth site; a larger candidate pool would commit it). On the live backend the portfolio cools a **measured 24,356 m²**, adds **5,412 m² of mature canopy**, and serves approximately **22,590 residents** within the unioned 300 m catchments (de-duplicated). The species mix is *Tipuana tipu* (40), *Celtis australis* (33) and *Styphnolobium japonicum* (26) — zero London-plane, zero invasive.

We state the portfolio's epistemic status precisely. Its **placement is real** (building-, street- and furniture-aware geometry on every site), its **population served is real** (municipal register and neighbourhood geometry), and its **site prioritisation is real** (the satellite vulnerability composite). As of the measured run (`tools/precompute_portfolio.py`), its **per-site cooling is also measured**: each funded cell was evaluated on the live Infrared UTCI backend (`cooling_source = "measured_utci"`), the funding order is by measured cost-per-m²-cooled (€32–41/m²), and the results are cached by geometry hash for offline replay. The measured portfolio funds 7 sites and 99 trees for €990,000 (the geographic-spread constraint exhausted the candidate pool before the final €10,000 could be committed to a sufficiently distant eighth site), cools a measured 24,356 m², and serves 22,590 residents. Between the two backends the system exposes a three-tier `CoolingEstimator` (mock scalar → sim-free shade proxy → cached/live measured UTCI), so a sim-free build still produces a geometry-grounded *estimate* clearly badged as such, while the committed artifact is measured.
...
2. **Coverage proxy versus measured cooling** *(remediated; see `coolspend/smart_placement.py`).* The greedy now weights each (slot, species)→cell pair by ray-cast **shade-gain** — the fraction of sampled suns whose crown shadow lands on the cell — rather than binary crown overlap, with per-cell captured gain capped so double-shading earns nothing. Placement *order* is therefore driven by where shadow actually falls, the objective stays submodular (preserving the (1 − 1/e) guarantee), and a live A/B confirmed it improves the headline (2,779 m² cooled at €72.0/m² vs the binary layout's 2,573 m² at €77.7/m²). The residual is epistemic, not procedural: shade-gain is a first-order direct-beam proxy, so the cooling *magnitude* is still taken from the live UTCI validation, by design.

3. **Synthetic cooling on non-live backends** *(remediated; see `tools/precompute_portfolio.py` and `coolspend/cooling_estimator.py`).* The headline €1,000,000 portfolio is now assembled from **live Infrared UTCI** (`cooling_source = "measured_utci"`), cached by geometry hash for offline replay, and funded by measured €/m²-cooled. For sim-free builds, a three-tier `CoolingEstimator` provides a geometry-grounded shade-proxy *estimate* in place of the old uniform-demand preview, with the measured-vs-estimate distinction enforced by an `is_measured` flag and surfaced as a badge in the UI rather than buried in prose. The residual honesty point is that the *cheapest* sim-free tier remains an estimate, by design.

4. **Surrogate thermal relief in NSGA-II** *(remediated; see `coolspend/data/calibration_summary.json`).* The assumed ±4 °C uncertainty is replaced by an empirical **±0.78 °C** band measured in a live calibration study (1.96 × RMSE), and the optimizer now reports this calibrated band rather than the hardcoded 4.0. The study also quantified the surrogate's character honestly — R² = −0.946 (weak magnitude) but Spearman ρ = 0.69 (moderate ranking) — confirming the design choice to use the surrogate only for ordering and to take magnitude from the live sim. The former "unsourced 12 °C cap" is documented as a non-binding guard: the calibration shows the surrogate never predicts above ~0.5 °C site-averaged ΔTmrt in practice, so the cap is never reached and is not a magnitude claim. The remaining refinement is to replace the surrogate's internal physics with the shade-proxy ray-cast (the abstraction is already available); the *honesty* of the band and cap is now closed.
...
7. **Canopy-cover denominator** *(remediated; see `coolspend/design_metrics.py`).* Canopy cover is now reported against **two** denominators: the sampled cell area (comparable city-wide) and the *plantable strip* — the site boundary minus buildings and street/furniture buffers, computed from the placement geometry already assembled. The headline cover uses the plantable strip (the truer local intensity), records which denominator was used, and keeps the ambient-ΔT estimate keyed to the cell cover (the 0.075 °C/% coefficient is a neighbourhood-scale relationship). This corrects the previously understated local figure.
