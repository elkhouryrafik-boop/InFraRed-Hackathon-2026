...
**Provenance limitation.** The cell attributes and their satellite origins are documented in code, but the detailed derivation of `composite_score_B` — the exact fusion weights combining Landsat heat with Sentinel-1 sealing, the LST anomaly baseline, and the SAR-to-sealed classifier — is not fully documented within the analysed codebase and appears to live in an upstream ingestion pipeline. We flag this as the single largest provenance gap in the system (Section 9).
...
1. **Satellite-composite provenance.** The derivation of `composite_score_B` — the fusion weights, the LST anomaly baseline, the SAR-to-sealed classifier and its training data — is not fully documented within the analysed system and resides in an upstream pipeline. Site *prioritisation* therefore rests on a signal we can describe but not, from this codebase alone, fully reproduce. Closing this gap is the highest-priority provenance task.
...
The system integrates publicly available data: the Barcelona municipal population register (*Padró*, dataset `pad_mdbas`) and street-tree inventory (*arbrat viari*) from the Barcelona Open Data portal; neighbourhood boundary geometry from an open GeoJSON mirror; building, road and street-furniture geometry from OpenStreetMap via the Overpass API; and satellite-derived heat, imperviousness and vegetation layers (Landsat, Sentinel-1, Sentinel-2). UTCI fields are produced by the third-party Infrared.city engine, which requires an API key and is not redistributable. The satellite vulnerability composite (`scored_grid`) is a derived product whose full derivation pipeline is external to the analysed codebase (see Limitations). Project source code and cached non-proprietary artifacts are held in the project repository.
